...
b = 0.03                    # [m] length of side flange
t1 = 0.005                  # [m] thickness of side flange
t2 = 0.005                  # [m] thickness of bottom flange

n_ribs = 0                   # [-] number of ribs
t_ribs = 0.01               # [m] thickness of ribs

n_stringer_decr_ribs = 0    # [-] decrease in number of stringers per x ribs
stringer_decr_x_ribs = 0    # [-] decrease stringers by n_stringer_decr_ribs every x ribs
...
while failure == False:
    for rib_number, j in enumerate(spanwise_pos_sec):
        for i in j:
            ...
            Ix_s_t, Iy_s_t  = L_stringer(a, b, t1, t2, angle_top)[3:5]
            Ix_s_b, Iy_s_b  = L_stringer(a, b, t1, t2, angle_bot)[3:5]
            ...
            if i == spanwise_pos_sec[rib_number][0]:
                ...
                # # stress due to normal force
                # sigma_n = loading_tension[0][np.where(spanwise_pos == i)[0][0]]/(l_top*t_top+l_bot*t_bot+l_f_spar*t_f_spar+l_a_spar*t_a_spar+(n_stringers+2)*A_s)
                ...
                if i == spanwise_pos[0]:
                    ...
                    sigma_cr_top_s = K*np.pi**2*material[mat]['E']*10**9*((n_stringers_top+2)*Ix_s_t)/((ac.b/2)**2*(n_stringers_top+2)*A_s)
                    sigma_cr_bot_s = K*np.pi**2*material[mat]['E']*10**9*((n_stringers_bot+2)*Ix_s_b)/((ac.b/2)**2*(n_stringers_bot+2)*A_s)
...
# Calculate weight
weight = trapezoid(As, spanwise_pos)*material[mat]['density']
