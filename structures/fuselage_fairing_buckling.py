...
def fuselage_fairing(aircraft):
    ...
    aircraft.W_fus_fairing_sides = main_sides_mass

    bottom_mass = (t_sheet * b*3 * w + 4*a*8*t_stringer*b*3 + 24*a*8*t_stringer*0.45 - t_sheet*0.45*0.45*12) * rho_GFRP
    aircraft.ST_W_fus_fairing_bot = bottom_mass

    top_mass = (3*b*w*t_sheet) * rho_GFRP
    aircraft.ST_W_fus_fairing_top = top_mass

    tail_mass = 1.418 * t_nose_tail * rho_GFRP
    aircraft.ST_W_fus_fairing_tail = tail_mass

    nose_mass = 2.916 * t_nose_tail * rho_GFRP
    aircraft.ST_W_fus_fairing_nose = nose_mass

    mass_total = main_sides_mass + bottom_mass + top_mass + tail_mass + nose_mass + 3
    aircraft.ST_W_fus_fairing_tot = mass_total

    aircraft.ST_W_fus = aircraft.ST_W_fus_fairing_tot + aircraft.ST_W_fus_truss