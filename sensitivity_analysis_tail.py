...
print(f'convential wins {np.count_nonzero(all_random_winner == 1)} out of {5 * iterations} times, which is {np.count_nonzero(all_random_winner == 1)/50000 * 100}%')
print(f'v tail wins {np.count_nonzero(all_random_winner == 2)} out of {5 * iterations} times, which is {np.count_nonzero(all_random_winner == 2)/50000 * 100}%')
print(f'inverted v tail wins {np.count_nonzero(all_random_winner == 3)} out of {5 * iterations} times, which is {np.count_nonzero(all_random_winner == 3)/50000 * 100}%')
print(f'y tail wins {np.count_nonzero(all_random_winner == 4)} out of {5 * iterations} times, which is {np.count_nonzero(all_random_winner == 4)/50000 * 100}%')
print(f'h tail wins {np.count_nonzero(all_random_winner == 5)} out of {5 * iterations} times, which is {np.count_nonzero(all_random_winner == 5)/50000 * 100}%')
print(f'flying tailplane wins {np.count_nonzero(all_random_winner == 6)} out of {5 * iterations} times, which is {np.count_nonzero(all_random_winner == 6)/50000 * 100}%')
print('-----------------------------------------------------------')